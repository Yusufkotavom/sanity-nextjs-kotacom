...
- [x] Shared GROQ `blocks` fragment now includes SEO block types (`company-info`, `testimonials-block`, `pricing-block`, `faq-block`), restoring schema-query-renderer parity for CMS block flows.
- [x] Repo-level operational architecture map for frontend + Sanity + agent workflows documented as source-of-truth (`docs/repo-frontend-sanity-architecture-agent-guide.md`) for future implementation planning and doc-sync checks
...
- [x] Repo-level operational architecture guide for frontend + Sanity + agent workflows published (`docs/repo-frontend-sanity-architecture-agent-guide.md`) with explicit schema/query/renderer sync guardrails.

### A1. CMS Contract Freeze
- [x] Synced shared frontend `blocks` GROQ contract with Studio SEO block schema to prevent dropped SEO block payloads in page/content rendering paths.
