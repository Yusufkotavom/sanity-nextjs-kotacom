...
async function main() {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (decision?.includes('approved_redirect') && targetPath.startsWith('/products/')) {
      const parts = targetPath.split('/').filter(Boolean);
      
      if (parts.length === 2) {
        // e.g. /products/pc-rakitan
        const slug = parts[1];
        if (!existingSlugs.has(slug)) {
          productsToCreate.set(slug, {
            _type: 'product',
  ...
  console.log(`Found ${productsToCreate.size} unique Products and ${categoriesToCreate.size} Product Categories to create.`);

  const allDocs = [...productsToCreate.values(), ...categoriesToCreate.values()];
  if (allDocs.length === 0) {
    console.log('No new products/categories to create.');
    return;
  }
