...
async function main() {
  ...
  const servicesToCreate = new Map();
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (decision?.includes('approved_redirect') && targetPath.startsWith('/services/')) {
      const parts = targetPath.split('/').filter(Boolean);
      
      if (parts.length === 2) {
        // e.g. /services/jasa-pembuatan-website
        const slug = parts[1];
        if (!existingSlugs.has(slug)) {
          servicesToCreate.set(slug, {
            _type: 'service',
        ...
        // e.g. /services/category/it-support
        const slug = parts[2];
        if (!existingSlugs.has(slug)) {
  ...
  console.log(`Found ${servicesToCreate.size} unique Services and ${categoriesToCreate.size} Service Categories to create.`);

  const allDocs = [...servicesToCreate.values(), ...categoriesToCreate.values()];
  if (allDocs.length === 0) {
    console.log('No new services/categories to create.');
    return;
  }
  ...
  try {
    ...
    console.log(`✅ Successfully created ${allDocs.length} drafts in Sanity.`);
  } catch (err) {
